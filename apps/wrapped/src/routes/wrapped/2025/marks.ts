...
import { schema, wrappedUsers, wrapped2025Items, wrapped2025Marks, wrappedCollectionTasks } from "@/db";

const marks = new Hono<{ Bindings: CloudflareBindings; Variables: AuthContext }>();
 ...
 * POST /collect/start
 */
marks.post("/collect/start", jwtAuth, async (c) => {
 ...
 * POST /collect/next/:taskId
 */
marks.post("/collect/next/:taskId", jwtAuth, async (c) => {
    const user = c.get("user");
 ...
 * GET /collect/status/:taskId
 */
marks.get("/collect/status/:taskId", jwtAuth, async (c) => {
    const user = c.get("user");
    const taskId = c.req.param("taskId");
    const db = drizzle(c.env.WRAPPED_DB, { schema });
 ...
 * POST /finalize/:taskId
 */
marks.post("/finalize/:taskId", jwtAuth, async (c) => {
    const user = c.get("user");
    const taskId = c.req.param("taskId");
    const db = drizzle(c.env.WRAPPED_DB, { schema });
